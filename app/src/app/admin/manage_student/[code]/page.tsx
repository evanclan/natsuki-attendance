...
export default function StudentDetailPage() {
    ...
    const handleSaveMemo = async () => {
        ...
        setSaving(true)
        const result = await updateStudentMemo(code, memo)
    ...
    const handleSaveAttendance = async (recordId: number) => {
        ...
        setSaving(true)
        const result = await updateAttendanceRecord(recordId, student.id, {
            check_in_at: toISO(editForm.check_in_at),
            check_out_at: toISO(editForm.check_out_at),
            break_start_at: null, // We're not editing breaks yet for simplicity, or preserve original?
            // Ideally we should preserve original breaks if we don't edit them.
            // For now, let's just pass null to reset them or we need to fetch them.
            // The action calculates work time based on in/out.
            // Let's assume we want to keep breaks if we don't touch them?
            // The current action implementation resets breaks if not provided.
            // Let's update the action to handle partial updates or just pass existing breaks?
            // Actually, let's just pass null for breaks for now as the UI doesn't support editing them yet.
            // Wait, if I pass null, it might clear existing breaks.
            // Let's look at the record being edited.
            break_end_at: null,
                    ...
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Textarea
                                ...
                                placeholder="Add notes or comments about this student..."
                                rows={6}
                            />
                        </div>
                        <div className="flex justify-end">
